    ...
    with open(os.path.join(extract_dir, "Scanner", coord, f"{coord}_inputs_distribution.json"), "r") as file:
        ...
        if times_val1:
            assert distrib['distrib'][
                       str(times_val1[0])] == times_val1[1], f"Value {times_val1[0]} expected {times_val1[1]} times, got {distrib['distrib'][times_val1[0]]}"
        if times_val2:
            assert distrib['distrib'][
                       str(times_val2[0])] == times_val2[1], f"Value {times_val2[0]} expected {times_val2[1]} times, got {distrib['distrib'][times_val2[0]]}"
...
def test_run_cj_process_ww1():
    ...
    # Run initial processing
    #
    run_parse_dumplings("ww1", "process_dumplings", f"interval_start_date={interval_start_date};interval_stop_date={interval_stop_date}", extract_dir)
    # ASSERT
    ...
    # Run false positives detection
    #
    run_parse_dumplings("ww1", "detect_false_positives", f"interval_start_date={interval_start_date};interval_stop_date={interval_stop_date}", extract_dir)
    # ASSERT
    for coord in ["wasabi1"]:
        with open(os.path.join(extract_dir, "Scanner", coord, "no_remix_txs.json"), "r") as file:
            results = orjson.loads(file.read())
            assert len(results['inputs_noremix']) == 12, f"Expected {12} no inputs remix coinjoins, got {len(results['inputs_noremix'])}"
            assert len(results['outputs_noremix']) == 6, f"Expected {6} no outputs remix coinjoins, got {len(results['outputs_noremix'])}"
            assert len(results['both_noremix']) == 2, f"Expected {2} both no remix coinjoins, got {len(results['both_noremix'])}"
            assert len(results['specific_denoms_noremix_in']) == 0, f"Expected {0} specific denoms noinput in, got {len(results['specific_denoms_noremix_in'])}"
            assert len(results['specific_denoms_noremix_out']) == 0, f"Expected {0} specific denoms noinput out, got {len(results['specific_denoms_noremix_out'])}"
            assert len(results['specific_denoms_noremix_both']) == 0, f"Expected {0} specific denoms noinput both, got {len(results['specific_denoms_noremix_both'])}"
            assert len(results['inputs_address_reuse_0_70']) == 0, f"Expected {0} input address reuse, got {len(results['inputs_address_reuse_0_70'])}"
            assert len(results['outputs_address_reuse_0_70']) == 0, f"Expected {0} output address reuse, got {len(results['outputs_address_reuse_0_70'])}"
    ...
    run_parse_dumplings("ww1", "split_coordinators", f"interval_start_date={interval_start_date};interval_stop_date={interval_stop_date}", extract_dir)
    # TODO: ASSERT 'txid_coord_discovered_renamed.json'
    ...
    # Analyze liquidity
    #
    run_parse_dumplings("ww1", None, f"ANALYSIS_LIQUIDITY=True;interval_start_date={interval_start_date};interval_stop_date={interval_stop_date}", extract_dir)
    # ASSERT

    expected_results = {
        "wasabi1_zksnacks": {"total_fresh_inputs_value": 934.54815701, "total_friends_inputs_value": 0.0,
            "total_unmoved_outputs_value": 838.22085515, "total_leaving_outputs_value": 96.31540543, "total_nonstandard_leaving_outputs_value": 39.21255878,
        ...
        "wasabi1_others": {"total_fresh_inputs_value": 1800.87207463, "total_friends_inputs_value": 0.0,
            "total_unmoved_outputs_value": 1118.58730176, "total_leaving_outputs_value": 489.54757815, "total_nonstandard_leaving_outputs_value": 473.459393,
    ...
    # ASSERT
    file_check = check_coinjoin_files(os.path.join(extract_dir, 'Scanner'))
    assert len(file_check['results']['wasabi1']['mix_base_files']['missing_files']) == 0, f"Missing files: {file_check['results']['wasabi1']['mix_base_files']['missing_files']}"
    assert len(file_check['results']['wasabi1_zksnacks']['mix_base_files']['missing_files']) == 5, f"Missing files: {file_check['results']['wasabi1_zksnacks']['mix_base_files']['missing_files']}"
