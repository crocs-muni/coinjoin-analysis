...
def generate_llm_inputs(cjtx_stats, WASABIWALLET_DATA_DIR):
    ...
    for start_cjtxid in cjtx_stats['coinjoins'].keys():
        ...
        # Number all inputs for start transaction incrementally, join inputs and corresponding outputs
        # for index in coinjoins_dupl[start_cjtxid]['inputs']:
        #     coinjoins_dupl[start_cjtxid]['inputs'][index]['coin_counter'] = coin_counter
        #     coin_counter += 1
        # for index in coinjoins_dupl[start_cjtxid]['outputs']:
        #     coinjoins_dupl[start_cjtxid]['outputs'][index]['coin_counter'] = coin_counter
        #     coin_counter += 1
        ...
        for input_variant in range(0, num_inputs_first_tx):
            for output_variant in range(0, num_outputs_last_tx):
                ...
                test_item['first_wallet'] = first_input_wallet
                test_item['last_wallet'] = last_output_wallet
    ...
    if False:
        ...
        for cjtxid in cjtx_stats['coinjoins'].keys():
            for index in cjtx_stats['coinjoins'][cjtxid]['inputs'].keys():
                value = int(cjtx_stats['coinjoins'][cjtxid]['inputs'][index]['value'] * SATS_IN_BTC)
                wallet_name = cjtx_stats['coinjoins'][cjtxid]['inputs'][index]['wallet_name'].replace('SimplePassiveWallet', 'w')
                # inputs are inserted as positive values
                events_vector.append((value, wallet_name))

            # use zero as separator between inputs and outputs
            events_vector.append((0, '|'))

            for index in cjtx_stats['coinjoins'][cjtxid]['outputs'].keys():
                value = int(cjtx_stats['coinjoins'][cjtxid]['outputs'][index]['value'] * SATS_IN_BTC)
                # outputs are inserted as negative values
                if 'wallet_name' in cjtx_stats['coinjoins'][cjtxid]['outputs'][index].keys():
                    wallet_name = cjtx_stats['coinjoins'][cjtxid]['outputs'][index]['wallet_name'].replace('SimplePassiveWallet', 'w')
                    events_vector.append((-value, wallet_name))
                else:
                    print('ERROR: missing wallet_name for {} - {}'.format(cjtxid, index))
                    events_vector.append((-value, 'unknown'))

            # use two zeros as separator between coinjoins
            events_vector.append((0, '#'))
        return {'events': events_vector}
